...
import { BandSegments } from '@/components/dashboard/band-segments';
import {
  CalibrationNote,
  ConfidenceMeaning,
...
import { FlagGroups } from '@/components/dashboard/flag-groups';
import { ManipulationTable } from '@/components/dashboard/manipulation-table';
import { MethodologyBlock } from '@/components/dashboard/methodology-block';
...
import { BandTimeline } from '@/components/dashboard/band-timeline';
...
import { isLowerBound } from '@/lib/keel/format/flags';
import { classify, classifyCount } from '@/lib/keel/format/value';
      ...
      <PageHeader title="What depth stands behind this price?">
        {risk ? (
          <p>
            ...
            against <span className="tabular">{risk.quote.code}</span>
            {risk.asset.issuer ? (
              <>
                {' · '}
        ...
        <div className="mt-6">
          ...
          {risk.oracleResistance ? (
            ...
            <p className="mt-2 text-sm text-[var(--keel-muted)]">
              Not reported for this asset. Without it there is no comparison
              between the cost of an attack and the genuine trading it would
              have to hide inside.
            </p>
          )}
        </div>
      ...
      {failed !== null ? (
        ...
        <>
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-[var(--keel-muted)]">
              What an order can absorb before the price moves against a buyer.
              The rungs are nested, so one scale covers all three, and the lines
              darken as the rung widens.
            </p>
            <TrendChart
              className="mt-3"
              unit={quoteCode}
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-[var(--keel-muted)]">
              The series carries one manipulation rung, at the half per cent
              delta the engine treats as critical. A reading the engine could
              not produce breaks the line rather than sitting at zero.
            </p>
            <TrendChart
              className="mt-3"
              unit={quoteCode}
...
function reachRows(risk: AssetRisk, quoteCode: string): FigureRow[] {
  const pooled =
    risk.poolSpotPrice !== null && risk.poolSpotPrice !== undefined;
