    ...
    <>
      ...
      <header className="sticky top-0 z-40 border-b border-[var(--keel-border)] bg-[var(--keel-surface)]/95 backdrop-blur">
        <div className="mx-auto flex w-full max-w-[1400px] flex-wrap items-center gap-x-6 gap-y-2 px-4 py-3">
          {/* The wordmark leaves the dashboard. It used to be the asset list, back when
              the dashboard was the whole application and `/` was the set; now `/` is the
...
export function PageHeader({
  ...
  return (
    <div className="mb-6">
      ...
      {children ? (
        <div className="mt-2 max-w-3xl text-sm text-[var(--keel-muted)]">
          {children}
        </div>
      ) : null}
    </div>
...
export function Section({
  ...
  return (
    ...
    <section id={id} className={cn('min-w-0 scroll-mt-20', className)}>
      ...
      {standfirst ? (
        <div className="mt-1 max-w-3xl text-sm text-[var(--keel-muted)]">
          {standfirst}
        </div>
      ) : null}
