...
export function CollateralCeilingPanel({
  ceiling,
  className,
}: CollateralCeilingProps) {
  ...
  return (
    <div className={cn('flex flex-col gap-4', className)}>
      ...
      <dl className="grid gap-px overflow-hidden rounded-lg border border-[var(--keel-border)] bg-[var(--keel-border)] sm:grid-cols-2">
        ...
        >
          {ceiling.manipulationTerm === 'not-applicable' ? (
            ...
            <span className="text-base text-[var(--keel-muted)]">
              not applicable
            </span>
          ) : (
      ...
      {binding === 'unmatched' ? (
        <Breach>
          The engine served a ceiling that equals neither term. The contract
          defines the ceiling as the minimum of the two, so one of the three
          figures on this page disagrees with the others. None of them has been
          adjusted here.
        </Breach>
      ) : null}
      ...
      {ceiling.missingLiquidationTerm ? (
        <Breach>
          The engine served a ceiling with no liquidation term. The contract
          says the liquidation term is always present when the ceiling is.
        </Breach>
      ) : null}
    </div>
  );
...
> = {
  ...
  unmeasured:
    'Not computed, along with the ceiling itself. See the engine notes below.',
};
