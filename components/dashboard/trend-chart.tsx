...
export function TrendChart({
  ...
  const plotted = series.map((s) => ({
    ...
    latest: [...s.points]
      .reverse()
      .find((p) => p.value !== null && p.value !== undefined),
  }));
      ...
      {bounds === null ? null : (
        <dl className="mt-1 flex justify-between text-xs text-[var(--keel-muted)]">
          <div className="flex items-baseline gap-1.5">
            ...
            <dd>
              <Value
                value={classify(bounds.min, unit)}
          ...
          <div className="flex items-baseline gap-1.5">
            ...
            <dd>
              <Value
                value={classify(bounds.max, unit)}
                maxFractionDigits={maxFractionDigits}
              />
            </dd>
          </div>
      ...
      <dl className="mt-2 flex flex-wrap gap-x-6 gap-y-1 text-sm">
        {plotted.map((s) => (
          <div key={s.key} className="flex items-baseline gap-2">
            ...
            <dd>
              <Value
                value={classify(s.latest?.value, unit)}
                maxFractionDigits={maxFractionDigits}
              />
            </dd>
          </div>
