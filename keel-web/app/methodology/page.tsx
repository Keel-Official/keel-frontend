...
import { ThresholdTable } from '@/components/keel/threshold-table';
import { AppShell, PageHeader, Section } from '@/components/layout/app-shell';
import { fetchHealth, fetchMethodology } from '@/lib/api/server';
...
export default async function MethodologyPage() {
  const [health, methodology] = await Promise.all([fetchHealth(), fetchMethodology()]);
    ...
    >
      <PageHeader title="Which thresholds produced these numbers?">
        <p>
          ...
          response, so what is on screen is what was applied. No threshold is written
          into this dashboard, and the list below is whatever the engine served rather
      ...
      {methodology.failure ? (
        ...
        <Notice tone="empty" title="The engine returned no methodology document" />
      ) : (
...
function MethodologyView({
  ...
  const calibration = calibrationStatement(served.calibrated, served.calibrationNote);
    ...
    <div className="flex flex-col gap-10">
      ...
      */}
      ...
      <Section
        ...
        standfirst={
          <>
            Keys are shown verbatim, because the key name is what a consumer looks up in
            the contract. Every key ending in <code className="tabular">Pct</code> is a
            percentage rather than a fraction — that is the one unit convention the
            contract states, and it is the only one applied here.
          </>
        }
      >
        {groups.length === 0 ? (
          <Notice tone="empty" title="The engine served no thresholds in this response" />
        ) : (
          <ThresholdTable groups={groups} />
        )}
        <p className="mt-3 text-xs text-[var(--keel-muted)]">
          A threshold denominated in an asset is shown with its full code and issuer. A
          bare ticker would name nothing: ninety-seven distinct assets share the AQUA
          ticker alone.
        </p>
      </Section>
    </div>
