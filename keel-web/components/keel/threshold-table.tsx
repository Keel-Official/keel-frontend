import type { AssetIdValue, ThresholdGroup } from '@/lib/format/methodology';
    ...
    <div className={cn('relative overflow-x-auto rounded-lg border border-[var(--keel-border)]', className)}>
      <table className="w-full border-collapse text-sm">
        <caption className="sr-only">
          ...
          denominated in where the engine reports one.
        </caption>
        <thead>
          ...
          {groups.map((group) => (
            ...
            >
              <th scope="row" className="px-3 py-2 text-left font-normal">
                <code className="tabular text-[var(--keel-ink-strong)]">{group.key}</code>
                {group.denominatedIn ? (
...
function ThresholdValueCell({ group }: { group: ThresholdGroup }) {
  ...
  if (value.kind === 'opaque') {
    return <code className="tabular break-all text-[var(--keel-ink)]">{value.text}</code>;
  }
...
function AssetPair({ asset }: { asset: AssetIdValue }) {
  return (
    <span className="tabular text-[var(--keel-ink)]" title={asset.raw}>
      {asset.code}
      <span className="text-[var(--keel-muted)]">:{truncateIssuer(asset.issuer, 6)}</span>
    </span>
  );
}
