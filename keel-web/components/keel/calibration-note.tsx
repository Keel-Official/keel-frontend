  ...
  if (calibrated === undefined) {
    return (
      <p className={cn('text-xs text-[var(--unmeasured)] italic', className)}>
        The methodology document could not be read, so the calibration status of these
        bands is unknown.
      </p>
    );
  }
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 text-sm">
        ...
        {note ? (
          <p className={cn('mt-1 text-[var(--keel-ink)]', variant === 'compact' && 'text-xs')}>
            {note}
          </p>
        ) : null}
...
export function ConfidenceMeaning({ className }: { className?: string }) {
  return (
    <p className={cn('text-xs text-[var(--keel-muted)]', className)}>
      A band marked <span className="font-medium">partial</span> is a floor. At least one
      high-severity check could not be evaluated, so the real band can only be worse than
      the one shown, never better.
    </p>
  );
}
