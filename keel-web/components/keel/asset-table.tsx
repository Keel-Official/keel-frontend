...
import { assetKey } from '@/lib/assets/list';
...
import { sortHref, type AssetQuery, type SortKey } from '@/lib/url/asset-query';
          ...
          <tbody>
            {items.map((item) => (
              ...
              >
                ...
                <td className="px-3 py-2 text-right tabular">{item.flags.length}</td>
              </tr>
            ))}
          </tbody>
        </table>
...
function AssetIdentity({ asset }: { asset: AssetSummary }) {
  ...
  return (
    <div className="min-w-0">
      <Link
        href={`/asset/${encodeURIComponent(id)}`}
      ...
      {asset.asset.issuer ? (
        <div className="tabular text-xs text-[var(--keel-muted)]" title={asset.asset.issuer}>
