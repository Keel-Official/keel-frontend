...
function compareBy(key: SortKey, a: AssetSummary, b: AssetSummary): number {
  switch (key) {
    ...
    case 'depth':
      return byValue('asc')(classify(a.depth5PctBuySide), classify(b.depth5PctBuySide));
    case 'collateral':
      return byValue('asc')(classify(a.maxSafeCollateral), classify(b.maxSafeCollateral));
    case 'flags':
      return compareDecimalStrings(String(a.flags.length), String(b.flags.length));
  }
}
...
function missing(key: SortKey, asset: AssetSummary): boolean {
  const raw = key === 'depth' ? asset.depth5PctBuySide : asset.maxSafeCollateral;
